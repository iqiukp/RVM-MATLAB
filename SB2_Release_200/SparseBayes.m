...
% Modified on 11st May 2019, by Kepeng Qiu.
if nargout>=1
    ...
    PARAMETER.Bias  = 0;
    % "free" basis
    if ~isempty(OPTIONS.freeBasis)
        indexBias	= find(PARAMETER.Relevant == OPTIONS.freeBasis);
    ...
    if nargout>=2
        ...
        if nargout>=3
            ...
            for m=1:size(BASIS,2)
                BASIS(:,m)	= BASIS(:,m)*BasisScales(m);
            end
            PHI=BASIS(:,PARAMETER.Relevant);
            U	= chol(PHI'*PHI*beta)+ diag(HYPERPARAMETER.Alpha);
            Ui	= inv(U);
            DIAGNOSTIC.Sigma		= Ui * Ui'; % 后验协方差
        end
    end
end
